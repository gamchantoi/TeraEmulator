...
namespace Tera.Services
{
    class FeedbackService : Global, IFeedbackService
    {
        ...
        {
            new SpCheckVersion(version).Send(connection);
        }

        public void OnAuthorized(IConnection connection)
        {
            //1725 EU
            new SpTest("C06100000000D92C0200").Send(connection);
            new SpTest("92B10100000001000000000001").Send(connection);
        }

        public void SendPlayerList(IConnection connection)
        {
            new SpCharacterList(connection.Account).Send(connection);
        ...
        public void SendCheckNameForUseResult(IConnection connection, string name, short type, bool result)
        {
            //1725 EU
            if (result)
                new SpTest("525A01").Send(connection);
            else
                new SpTest("525A00").Send(connection);
        }
        ...
        {
            new SpTest("FBB3010000000000000000").Send(connection); //1725 EU
            new SpTest("F4B60000000000000000000000000000D49E9F4F00000000").Send(connection); //1725 EU
            ...
            new SpTest("F3AD").Send(connection); //1725 EU

            QuestEngine.ResendQuestData(connection.Player);

            new SpCharacterCraftStats(connection.Player).Send(connection);

            new SpTest("07730500160001000000000000003C000000000016002200000000000000000022002E00FFFFFFFF000000002E003A00FFFFFFFF000000003A004600FFFFFFFF0000000046000000FFFFFFFF00000000").Send(connection); //1725 EU physical abnormals
            new SpTest("8CD8").Send(connection); //1725 EU
            new SpTest("16770000000000000000").Send(connection); //1725 EU

            new SpTest("BC5500004843").Send(connection); //1725 EU
            new SpTest("E0C406000000").Send(connection); //1725 EU
        }
        ...
        public void OnPlayerEnterWorld(IConnection connection, Player player)
        {
            ...
            new SpTest("8FCD00000000").Send(connection); //1725 EU
            new SpTest("0D9600000000").Send(connection); //1725 EU

            //new SpFriendList(pState.Player.Friends).Send(connection);
            //new SpFriendUpdate(pState.Player.Friends).Send(connection);

            new SpTest("A7F20000000000000000").Send(connection); //1725 EU
            new SpTest("03E70000000000000000").Send(connection); //1725 EU
            ...
            new SpTest("E1EB" + uidHex + "7CC4000001FFFFFF7F").Send(connection); //1725 EU
            ...
            player.LastOnline = Funcs.GetRoundedUtc();
        }
        ...
        public void SendCreatureInfo(IConnection connection, Creature creature)
        {
            ...
            if (player != null)
            {
                try
                {
                    ...
                    if (player.Mount != 0 && Data.Data.Mounts.ContainsKey(player.Mount))
                        new SpMountShow(player, Data.Data.Mounts[player.Mount].MountId, player.Mount).Send(connection);
                }
                catch(Exception e)
                {
