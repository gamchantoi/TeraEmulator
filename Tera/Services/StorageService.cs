...
namespace Tera.Services
{
    internal class StorageService : IStorageService
    {
        ...
        {
            player.Inventory.Items.Add(20, new StorageItem {ItemId = 125, Count = 5});
            player.Inventory.Items.Add(21, new StorageItem { ItemId = 8007, Count = 3 });

            switch (player.PlayerData.Class)
            {
                case PlayerClass.Warrior:
                    player.Inventory.Items.Add(1, new StorageItem { ItemId = 10001, Count = 1 });
                    player.Inventory.Items.Add(3, new StorageItem { ItemId = 15004, Count = 1 });
                    player.Inventory.Items.Add(4, new StorageItem { ItemId = 15005, Count = 1 });
                    player.Inventory.Items.Add(5, new StorageItem { ItemId = 15006, Count = 1 });
                    break;
                case PlayerClass.Archer:
                    player.Inventory.Items.Add(1, new StorageItem { ItemId = 10006, Count = 1 });
                    player.Inventory.Items.Add(3, new StorageItem { ItemId = 15004, Count = 1 });
                    player.Inventory.Items.Add(4, new StorageItem { ItemId = 15005, Count = 1 });
                    player.Inventory.Items.Add(5, new StorageItem { ItemId = 15006, Count = 1 });
                    break;
                case PlayerClass.Slayer:
                    player.Inventory.Items.Add(1, new StorageItem { ItemId = 10003, Count = 1 });
                    player.Inventory.Items.Add(3, new StorageItem { ItemId = 15004, Count = 1 });
                    player.Inventory.Items.Add(4, new StorageItem { ItemId = 15005, Count = 1 });
                    player.Inventory.Items.Add(5, new StorageItem { ItemId = 15006, Count = 1 });
                    break;
                case PlayerClass.Berserker:
                    player.Inventory.Items.Add(1, new StorageItem { ItemId = 10004, Count = 1 });
                    player.Inventory.Items.Add(3, new StorageItem { ItemId = 15001, Count = 1 });
                    player.Inventory.Items.Add(4, new StorageItem { ItemId = 15002, Count = 1 });
                    player.Inventory.Items.Add(5, new StorageItem { ItemId = 15003, Count = 1 });
                    break;
                case PlayerClass.Lancer:
                    player.Inventory.Items.Add(1, new StorageItem { ItemId = 10002, Count = 1 });
                    player.Inventory.Items.Add(3, new StorageItem { ItemId = 15001, Count = 1 });
                    player.Inventory.Items.Add(4, new StorageItem { ItemId = 15002, Count = 1 });
                    player.Inventory.Items.Add(5, new StorageItem { ItemId = 15003, Count = 1 });
                    break;
                ...
                case PlayerClass.Sorcerer:
                    player.Inventory.Items.Add(1, new StorageItem { ItemId = 10005, Count = 1 });
                    player.Inventory.Items.Add(3, new StorageItem { ItemId = 15007, Count = 1 });
                    player.Inventory.Items.Add(4, new StorageItem { ItemId = 15008, Count = 1 });
                    player.Inventory.Items.Add(5, new StorageItem { ItemId = 15009, Count = 1 });
                    break;

                case PlayerClass.Mystic:
                    player.Inventory.Items.Add(1, new StorageItem { ItemId = 10008, Count = 1 });
                    player.Inventory.Items.Add(3, new StorageItem { ItemId = 15007, Count = 1 });
                    player.Inventory.Items.Add(4, new StorageItem { ItemId = 15008, Count = 1 });
                    player.Inventory.Items.Add(5, new StorageItem { ItemId = 15009, Count = 1 });
                    break;
                ...
                case PlayerClass.Priest:
                    player.Inventory.Items.Add(1, new StorageItem { ItemId = 10007, Count = 1 });
                    player.Inventory.Items.Add(3, new StorageItem { ItemId = 15007, Count = 1 });
                    player.Inventory.Items.Add(4, new StorageItem { ItemId = 15008, Count = 1 });
                    player.Inventory.Items.Add(5, new StorageItem { ItemId = 15009, Count = 1 });
                    break;
        ...
        public bool AddItem(Player player, Storage storage, int itemId, int count, int slot = -1)
        {
            ...
            lock (storage.ItemsLock)
            {
                ...
                {
                    if (slot == -1)
                        storage.Items.Add(storage.GetFreeSlot(), new StorageItem {ItemId = itemId, Count = count});
                    else if (storage.Items.ContainsKey(slot))
                    {
                        ...
                        storage.Items.Add(slot, new StorageItem {ItemId = itemId, Count = count});
                }
                else
                {
                    if (slot != -1)
                    ...
                    else
                    {
                        ...
                        int canBeAdded =
                            itemsById.Values.Where(storageItem => storageItem.Count < stackSize).Sum(
                                storageItem => stackSize - storageItem.Count);

                        if (canBeAdded >= count)
                        {
                            foreach (var storageItem in itemsById.Values)
                            {
                                int added = Math.Min(stackSize - storageItem.Count, count);
                                storageItem.Count += added;
                                count -= added;
                                if (count == 0)
                                    break;
                            }
                        ...
                        else
                        {
                            ...
                            while (count > 0)
                            {
                                ...
                                StorageItem item = new StorageItem {ItemId = itemId, Count = added};
            ...
            lock (storage.ItemsLock)
            {
                ...
                if (maxStack > 1)
                {
                    for (int i = 20; i < 20 + storage.Size; i++)
                    {
                        ...
                        {
                            canStacked += maxStack - storage.Items[i].Count;

                            if (canStacked >= item.Count)
                                break;
                        }
                    }
                }

                if (canStacked < item.Count && GetFreeSlots(storage).Count < 1)
                    return false;
                ...
                if (canStacked > 0)
                {
                    for (int i = 20; i < 20 + storage.Size; i++)
                    {
                        ...
                        if (storage.Items[i].ItemId == item.ItemId)
                        {
                            int put = maxStack - storage.Items[i].Count;
                            if (item.Count < put)
                                put = item.Count;

                            storage.Items[i].Count += put;
                            item.Count -= put;
        ...
        public bool RemoveItem(Player player, Storage storage, int slot, int counter)
        {
            ...
            lock (storage.ItemsLock)
            {
                if (!storage.Items.ContainsKey(slot) || storage.Items[slot].Count < counter)
                    return false;

                if (storage.Items[slot].Count == counter)
                    storage.Items.Remove(slot);
                else if (storage.Items[slot].Count > counter)
                    storage.Items[slot].Count -= counter;
        ...
        public bool ContainsItem(Storage storage, int itemId, long counter)
        {
            lock (storage.ItemsLock)
            {
                for (int i = (storage.StorageType == StorageType.Inventory ? 20 : 0);
                    ...
                    {
                        if (storage.Items[i].ItemId == itemId && storage.Items[i].Count >= counter)
                            return true;
                    }
            }
            return false;
        ...
        public bool PlaceItemToOtherStorage(Player player, Player second, Storage from, int fromSlot, Storage to, int toSlot, int count)
        {
            ...
            if(player == second)
            {
                ...
                {
                    if (!PlaceItemToOtherStorage(player, player, to, toSlot, player.Inventory, player.Inventory.GetFreeSlot(), to.Items[toSlot].Count))
                        return false;
                }
                ...
                lock (from.ItemsLock)
                {
                    if(from.Items.ContainsKey(fromSlot))
                    {
                        ...
                        {
                            from.Items[fromSlot].Count -= count;
                            item = new StorageItem { ItemId = item.ItemId, Count = count };
                        }
                    }
                }
